class Solution {
    bool solve(string& s1, string& s2, string& s3, int s1_idx, int s2_idx, int s3_idx, vector<vector<vector<int>>>& dp){
        if(s1_idx == s1.size() && s2_idx == s2.size() && s3_idx == s3.size()) return true;
        if(dp[s1_idx][s2_idx][s3_idx] != -1) return dp[s1_idx][s2_idx][s3_idx];
        ...
        return dp[s1_idx][s2_idx][s3_idx] = (match1 || match2);
    }
public:
